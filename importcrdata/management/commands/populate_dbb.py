...
class Command(BaseCommand):
    ...
    def get_distelec(self,distelec):
        ...
        for ditelec_info in distelec:
            ...
            query = "INSERT INTO importcrdata_distelec values " + "('%s','%s','%s','%s','%s');" % (id, codelec,provincia,canton,distrito.strip())
            id += 1
            yield query
    ...
    def get_padron(self,padron):
        ...
        for padron_info in padron:
            ...
            nombre_completo = nombre.strip().replace("'",' ')+' '+apellido1.strip().replace("'",' ')+' '+apellido2.strip().replace("'",' ')
            query += 'INSERT INTO importcrdata_padronelectoral values' + "('%s','%s','%s',%s,'%s','%s','%s');" % (id, cedula, codelec, sexo, fechacaduc,'00000', nombre_completo)
            cont += 1
            id += 1
            if cont == 100000:
                yield query
